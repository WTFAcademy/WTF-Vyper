...
在 `Vyper` 中， `create_from_blueprint` 是一种创建新合约实例的方法。它利用存储在区块链上的初始化代码 `initcode`，从一个特定的“蓝图”合约部署新的合约实例。这种方法相对于直接部署新合约，可以节省gas成本，并允许通过蓝图合约调用构造函数
...
比如 `erc20` 合约中有4个构造参数，但是部署时不需要传递它，而是在 `create_from_blueprint` 执行时才需要传递
...
部署新的蓝图合约时传递了4个参数，部署完成后尝试调用蓝图合约的函数，可以看到现在是可以执行的
