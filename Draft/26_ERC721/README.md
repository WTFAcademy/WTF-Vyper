...
当查询的是 `ERC165` 或 `ERC721` 的接口ID时返回 `True`，反之返回 `False`
...
- `tokenURI`: 返回一个指定tokenID的 `URI`，该 `URI` 指向一个包含该 `NFT` 相关信息（如名称、描述、图像等）的 `JSON` 文件。这个 `JSON` 文件通常托管在 `IPFS` 或其他去中心化存储解决方案上
```
@view
@external
